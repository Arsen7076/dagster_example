...
YELOW_DATE_COLUMN_NAME = os.getenv("DATE_COLUMN_NAME", "tpep_pickup_datetime")
GREEN_DATE_COLUMN_NAME = os.getenv("DATE_COLUMN_NAME", "lpep_pickup_datetime")
...
def determine_partition_range(context):
    ...
    try:
        ...
        df = df.withColumn("date", to_date(df[YELOW_DATE_COLUMN_NAME]))
...
def create_backfill_asset(partitions_def):
    """
    Creates a backfill asset for historical data based on the provided partition definitions.
    """
    ...
    def backfill_historical_data(context):
        ...
        try:
            ...
            # Save partitioned DataFrame to a parquet file
...
def load_daily_partition(context):
    """
    Loads daily partitioned data from the green_tripdata parquet file.
    """
    partition_date = context.asset_partition_key_for_output()
